...
# GMP will be installed in /c/Software/MultPrecLibs
cd /c/Software/gmp-6.1.2-src/
./configure --enable-static --disable-shared --prefix=/c/Software/MultPrecLibs
make
make check
make install
make clean
...
# 1.25000E+00.
# MPFR will be installed in /c/Software/MultPrecLibs
cd /c/Software/mpfr-4.0.1-src/
# ./configure --prefix=/c/Software/MultPrecLibs --enable-static --enable-shared --with-gmp=/c/Software/MultPrecLibs CFLAGS='-D__USE_MINGW_ANSI_STDIO=1' LDFLAGS='-static'
./configure --prefix=/c/Software/MultPrecLibs --enable-static --disable-shared --with-gmp=/c/Software/MultPrecLibs
make
make check
make install
make clean
...
# Step  6) Tell python where to find the gcc, GMP and MPFR libraries. See https://gcc.gnu.org/onlinedocs/gcc/Environment-Variables.html
           PATH=C:\Software\msys64\mingw64\bin;%PATH%
           set LIBRARY_PATH=C:\Software\MultPrecLibs\lib
           set CPATH=C:\Software\MultPrecLibs\include
